...
class Chat:
    ...
    def converse(self, progress: ProgressFn | None = None) -> dict:
        """Hold one conversation: ``{"transcript", "exchanges", "turns", "stalled", "vetoed"}``.

        ``transcript`` is every line as ``(speaker, text)``, ``exchanges`` the
        ``(said to it, its reply)`` pairs the judge marks.  ``stalled`` is true
        when it ended early - the model had nothing left to say, or the partner
        went quiet.
        """
        from .dialogue import normalize, reply
        ...
        if not line:
            return {"transcript": [], "exchanges": [], "turns": [], "stalled": "the partner said nothing",
                    "vetoed": 0}
        transcript.append((DEFAULT_SPEAKERS[0], line))
        said.add(normalize(line))
        turns: list[Any] = []
        for index in range(cfg.turns):
            if self._stop.is_set():
                ...
                break
            turn = reply(
                self.model, line, said=said, index=len(transcript), speaker=DEFAULT_SPEAKERS[1], mode=cfg.mode,
            ...
            transcript.append((DEFAULT_SPEAKERS[0], line))
            said.add(normalize(line))
        return {"transcript": transcript, "exchanges": exchanges, "turns": turns, "stalled": stalled,
    ...
    def _learn(self, review: dict, held: dict) -> dict:
        ...
        bad = [text for text in dict.fromkeys(bad) if text not in good]
        result: dict[str, Any] = {"action": None, "bad": len(bad), "good": len(good),
                                  "neg_loss": None, "pos_loss": None}
