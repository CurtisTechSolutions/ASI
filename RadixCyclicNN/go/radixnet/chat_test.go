...
func TestChatAReplyItCouldOnlyRepeatIsPunishedWhateverTheJudgeSaid(t *testing.T) {
	loop, fake := newTestChat(t, nil, func(c *ChatConfig) {
		c.Learn, c.Turns, c.TeachPartner, c.NegEpochs, c.PosEpochs = true, 1, false, 1, 1
	})
	fake.passes = func(string) bool { return true } // the judge is happy with everything
	held := &ChatHeld{
		Transcript: []Line{{Speaker: ChatSpeakers[0], Text: "tell me about the cat"}},
		Repeats:    []string{"the cat sat on the mat"},
	}
	learned := loop.Learn(&ReviewResult{Good: []string{"the cat sat on the mat"}}, held)
	// a duplicate is never rewarded, however well it was marked
	if learned["action"] != "punish" || learned["bad"] != 1 || learned["good"] != 0 || learned["repeats"] != 1 {
		t.Fatalf("a repeated reply must be punished: %v", learned)
	}
}
