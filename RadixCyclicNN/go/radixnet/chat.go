...
func (c *Chat) Converse(progress func(map[string]any)) (*ChatHeld, error) {
	...
	held := &ChatHeld{Transcript: []Line{}, Exchanges: []Exchange{}, Turns: []*Turn{}}
	...
	for index := 0; index < cfg.Turns; index++ {
		...
		turn, err := c.Model.Reply(line, ReplyOptions{
			Said: said, Index: len(held.Transcript), Speaker: ChatSpeakers[1], Mode: cfg.Mode,
...
// passed (and the partner's own lines) rewarded.
func (c *Chat) Learn(review *ReviewResult, held *ChatHeld) map[string]any {
	...
	good = uniqueStrings(good)
	known := map[string]bool{}
	for _, text := range good {
		known[text] = true
	}
	kept := []string{}
	for _, text := range uniqueStrings(bad) {
		if !known[text] {
			kept = append(kept, text)
		}
	}
	bad = kept
	result := map[string]any{"action": nil, "bad": len(bad), "good": len(good), "neg_loss": nil, "pos_loss": nil}
	switch {
