...
func TestEncodingAndItsPreview(t *testing.T) {
	...
	hasKeys(t, info, "window", "stride", "overlap", "start_label", "end_label", "back_label", "configurable", "note")
	...
	if info["configurable"] != false { // the window is part of the model format, not a setting
		t.Fatalf("the window must not be advertised as settable: %v", info["configurable"])
	}
