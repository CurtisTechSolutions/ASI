...
Frontend: an "Ollama" tab with a connection card (URL, model list), "Corpus from a prompt" (generate → train / save as upload / hold as 2NRL bad or good), "Adversarial review" (ratings table, summary, apply as 2NRL with extra good files) and "Letter-level correction" (every correction as a diff — `<del>` what the editor struck out of the model's text, `<ins>` what it wrote instead, a caret where a change leaves nothing on that side — with the verdict, the reason and the note, and a *teach the negative network* checkbox with its severity).
