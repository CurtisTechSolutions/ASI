...
export default function ChatPanel({ status }) {
  const [topic, setTopic] = useStoredState("chat.topic", "");
  const [opening, setOpening] = useStoredState("chat.opening", "");
  const [persona, setPersona] = useStoredState("chat.persona", "");
  const [conversations, setConversations] = useStoredState("chat.conversations", "1");
  const [turns, setTurns] = useStoredState("chat.turns", "4");
  const [context, setContext] = useStoredState("chat.context", "12");
  const [maxLength, setMaxLength] = useStoredState("chat.maxLength", "60");
  const [mode, setMode] = useStoredState("chat.mode", "beam");
  const [threshold, setThreshold] = useStoredState("chat.threshold", "6");
  const [provider, setProvider] = useStoredState("chat.provider", "ollama");
  const [partnerModel, setPartnerModel] = useStoredState("chat.partnerModel", "");
  const [url, setUrl] = useStoredState("chat.url", "");
  const [guard, setGuard] = useStoredState("chat.guard", true);
  const [avoidWordRepeats, setAvoidWordRepeats] = useStoredState("chat.avoidWordRepeats", true);
  const [explore, setExplore] = useStoredState("chat.explore", "3");
  const [blame, setBlame] = useStoredState("chat.blame", true);
  const [learn, setLearn] = useStoredState("chat.learn", true);
  const [teachPartner, setTeachPartner] = useStoredState("chat.teachPartner", true);
