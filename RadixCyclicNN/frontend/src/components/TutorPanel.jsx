...
export default function TutorPanel({ status }) {
  const [provider, setProvider] = useStoredState("tutor.provider", "ollama");
  const defaults = providerDefaults(status, provider);
  const [url, setUrl] = useStoredState("tutor.url", "");
  const [model, setModel] = useStoredState("tutor.model", "");
  const [topic, setTopic] = useStoredState("tutor.topic", "everyday life");
  const [focus, setFocus] = useStoredState("tutor.focus", "");
  const [level, setLevel] = useStoredState("tutor.level", "beginner");
  const [words, setWords] = useStoredState("tutor.words", "3 to 6");
  const [brief, setBrief] = useStoredState("tutor.brief", "");
  const [rounds, setRounds] = useStoredState("tutor.rounds", "3");
  const [exercises, setExercises] = useStoredState("tutor.exercises", "5");
  const [attempts, setAttempts] = useStoredState("tutor.attempts", "1");
  const [mode, setMode] = useStoredState("tutor.mode", "dijkstra");
  const [length, setLength] = useStoredState("tutor.length", "20");
  const [maxLength, setMaxLength] = useStoredState("tutor.maxLength", "80");
  const [temperature, setTemperature] = useStoredState("tutor.temperature", "1.0");
  const [threshold, setThreshold] = useStoredState("tutor.threshold", "6");
  const [grammarWeight, setGrammarWeight] = useStoredState("tutor.grammarWeight", "0.6");
  const [drills, setDrills] = useStoredState("tutor.drills", "0");
  const [planCount, setPlanCount] = useStoredState("tutor.planCount", String(DEFAULT_PLAN_LESSONS));
  const [batches, setBatches] = useStoredState("tutor.batches", "1");
  const [adapt, setAdapt] = useStoredState("tutor.adapt", true);
  const [teachAnswer, setTeachAnswer] = useStoredState("tutor.teachAnswer", true);
  const [twonrlPer, setTwonrlPer] = useStoredState("tutor.twonrlPer", "round");
  const [diffCorrections, setDiffCorrections] = useStoredState("tutor.diffCorrections", true);
  const [blame, setBlame] = useStoredState("tutor.blame", false);
  const [variants, setVariants] = useStoredState("tutor.variants", "3");
  const [variantWeight, setVariantWeight] = useStoredState("tutor.variantWeight", "0.5");
  const [keepWeight, setKeepWeight] = useStoredState("tutor.keepWeight", "0.25");
  const [minWeight, setMinWeight] = useStoredState("tutor.minWeight", "0.25");
  const [negEpochs, setNegEpochs] = useStoredState("tutor.negEpochs", "2");
  const [posEpochs, setPosEpochs] = useStoredState("tutor.posEpochs", "3");
  const [negLr, setNegLr] = useStoredState("tutor.negLr", "0.5");
  const [posLr, setPosLr] = useStoredState("tutor.posLr", "0.1");
