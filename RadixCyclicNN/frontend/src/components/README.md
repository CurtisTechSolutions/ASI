...
| `ConversePanel.jsx` | Converse | the model talking to itself. Newest turn on top, stutter detection, "Explore" to back out of a repetition, and "Think before backing up" - the 💭 line under a turn is what it thought, and "Stream": the conversation as it happens, the turn being spoken live at the top with what it backed out of struck through |
| `ChatPanel.jsx` | Chat | an LLM conversing with the model and marking every reply (a reply that thought before backing up shows the thought too) |
| `ThinkPanel.jsx` | Think | the model thinks: one thought from the THINK sentinel per press, about a text or nothing in particular, with the questions it asked itself nested under it (`POST /api/think`) |
...
| `ModelSettingsPanel.jsx` | Model settings | what belongs to the model and is saved with it: **this model** (kind, encoding, size, the replay buffer), a **new model** in any kind and encoding (`POST /api/reset`, two clicks), the **score function** of whichever kind is active (`GET /api/model` → `POST /api/model/weights`; the sine model has none and says so), the **attention band** (`AttentionBandCard.jsx`), and the **encoder / decoder** — the unit, the n, the stride, the four sentinels, and a live preview that encodes a text, decodes it back and walks it through the graph's own (possibly merged) node labels (`GET /api/encoding`, `POST /api/encoding/preview`) |
| `AttentionBandCard.jsx` | Model settings | the attention band: where inside a gram a correction lands. On or off and the blur (`GET` / `POST /api/model/attention`), the band drawn over one gram with its units blurred as the band sees them, and a correction typed in previewed under both rules at the slider's blur (`POST /api/model/attention/preview`, which changes nothing) |
| `DynamicWindowCard.jsx` | Model settings | the dynamic window: the ladder of node sizes the graph is halved down and regrown up through (32, 16, 8, 4 and back). On or off, the top, the floor, where it stands and whether it steps by itself at the end of every training epoch (`GET` / `POST /api/model/window`), the ladder drawn as its rungs, and a Step button that steps it by hand (`POST /api/model/window/step`), with what the step did in one line |
...
| `GraphView.jsx` | Graph | the graph itself, drawn and navigable; the sentinels in gold |
