...
//! front doors (the command line and the HTTP routes) onto it.
...
pub fn cli(ctx: &Ctx) -> Result<(), String> {
    match ctx.args.action() {
        ...
        "" => Err("ollama needs an action: models, corpus, review or correct".to_string()),
        other => Err(format!(
            "unknown ollama action {other:?}; expected models, corpus, review or correct"
        )),
    }
}
