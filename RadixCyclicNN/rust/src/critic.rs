...
/// Runs `f` on the server's negative network, loading it on first use
/// ([`Service::with_negative`], which keeps the two locks in one order).
...
mod tests {
    ...
    fn the_server_loop_takes_each_lock_for_a_step() {
        ...
        assert!(blame > 0.0);
        assert!(
            !std::path::Path::new(&crate::cli::negative_path(&path)).exists(),
