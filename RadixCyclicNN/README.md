...
| It notices its own loop and explores out of it | A repeat is not just skipped: the words *before* the walk went round were said once and were the most likely thing to say, so the voice keeps exactly those, backs up to the point it went round, and searches again from there - which forces the walk to leave the loop where it started rather than ranking the same answers again. Nothing new? It backs up another word and looks wider, `--explore` times over (3 by default). What it caught itself saying, what it kept, how many paths it weighed and whether it found a way on ride on the turn as its `rethink` record, and the CLI and both tabs say it in a line: *caught itself saying "ha" twice; kept "ha " and found another way on in 3 path(s)*. |
...
a voice that catches itself repeating may back up and look for another way on -
each turn saying what it noticed and found - and "Punish duplicates" marks
