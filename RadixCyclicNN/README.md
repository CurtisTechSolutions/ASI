...
| Duplicates are avoided, and punished | A reply that was **said** before, that **adds** what an earlier reply added, or that merely **echoes** a line already spoken is skipped. When every candidate is a duplicate the best one is spoken and flagged `repeat`; saying that same duplicate again would only go round in circles, so the conversation ends there. The flagged utterances come back as `repeats`, and the Converse tab marks them 👎 so "Train on ratings" punishes them (the 2NRL negative phase) - the model is taught out of the duplicates it cannot avoid by itself. |
...
view, newest turn at the top - new turns are appended there and push the older
ones down, so nothing has to be scrolled to: an opening line, turns, context,
beam / sample, the two voices' names, the other kind in memory as the second
voice; Continue extends the conversation, turns are rated like samples, and
"Punish duplicates" marks the repeats the model could not avoid 👎 for the
2NRL negative phase), Score, 2NRL,
