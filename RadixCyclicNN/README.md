...
| Word n-grams | the same count / reward model over an alphabet whose symbols are **words** (`--encoding word:3:1`, the Words tab): a unit is a word rather than a character, so a gram is n words of text and compression turns a repeated phrase into one node. There is no vocabulary - a gram is text, so the alphabet is whatever the grams are made of, and it grows as training reads new words. Lengths, counts and scores are per word. |
...
D-075) keeps every one of those rules and changes only what a *unit* is.  Not
one of the graph's structural rules mentions a character, so a word n-gram model
is not a different model and not a different kind: it is one setting of the
encoding dial, so a gram is n words of text and the same search walks words
instead of characters.  Compression then does to word chains what it does to
character chains - a repeated phrase becomes **one node whose label is that
phrase**.

```bash
python -m radixnet --encoding word:3:1 train --data data/sample_corpus.txt --epochs 3
python -m radixnet --encoding word:3:1 predict --prefix 'the cat sat on' --length 6
python -m radixnet words --limit 20        # the alphabet it has read, most read first
make word-demo                             # train, predict, generate, list the alphabet
```

There is no vocabulary, and that is the point: a gram is text, so the alphabet a
graph knows is whatever its grams are made of.  It grows as training reads new
words and there is nothing to freeze, prune, learn or check on load; tokenising
is the whitespace split and nothing else (punctuation stays attached, case is
kept, and the rule is the Unicode `White_Space` property so the three ports cut
a text into the same words).  What it costs, deliberately: a word encoding
**normalises whitespace**, so a corpus whose whitespace carries meaning (source
code, base64, a waveform) belongs on a character encoding, which is why
`char:3:1` stays the default.  Everything counted in units is counted in
**words** - `--length 6` emits six words, `score`'s `per_char` is per word - and
`units` in `stats`, the API and the frontend says which.  Go and Rust take the
same `--encoding` and read and write the same `radixnet-count` file, whose
`encoding` block says how to read its labels.
