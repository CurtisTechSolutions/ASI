...
Self-contained experiments that do not belong to any one project directory.
`Research/experiments/` holds the code that tests the claims in the papers;
this directory holds work that starts here rather than in a paper.

**Standard library only** — no numpy, no torch, no gym — so every script runs
