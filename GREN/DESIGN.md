...
confidence threshold (§21.1). A system that plays before it understands is
...
|---|---|
| Solve games; to solve a game, understand it; to understand it, explore it | Four phases with a hard boundary before play: **retrieve** (§3, candidates from partial evidence), **explore** (§16, probes chosen to separate them), **induce** (§15, rules — only where the candidates disagree), **encode** (§18, the tree). Playing is not in this package at all. |
| Figure out which game we are playing by finding similar games | §3 is the spine. A partial signature retrieves a **candidate set** from the tree (§18.4); probes are chosen to split that set (§16.4); the nearest surviving neighbour's rule set is inherited as a prior and corrected only where it fails (§15). Identification costs `log2|G|` bits against a corpus of `G` games; induction from nothing costs `log2|Θ|`, which is three to four orders of magnitude worse (§3.1). |
| Clustering, based on what we know about the game so far | Evidence is **partial by default** (§3.2): most axes are unobserved most of the time. Retrieval branches over unknown axes and scores paths by bucket posterior weighted by information gain (§18.4), so the answer is always a ranked cluster with a confidence, never a single guess pretending to be certain. |
| You learn the rules by failing | The `Oracle` (§11) is the thing that says no. Every probe returns a `Verdict`, and an ILLEGAL verdict with a reason is worth more than a LEGAL one — it names a boundary, where a success only says "somewhere inside". |
| Failing *on purpose*, guessing | The probe policy maximises expected information gain **about which game this is** (§4). For a binary oracle that objective is provably maximised at `p(legal) = 0.5`, so **the target failure rate is 50%** — it falls out of the mathematics rather than being set as a knob. The probe that best splits the candidate set is the one half of them call legal and half call illegal (§16.4). |
| The compiler tells you "this ain't working, bro" | `CompilerOracle` / `InterpreterOracle` (§11.1). The diagnostic text is not discarded — it is the highest-bandwidth channel in the system, and §14 clusters the oracle's own words into a reason taxonomy that becomes the rule vocabulary. |
...
| It is a classification task and a learning task at once | Learning = rule induction (§15). Classification = which leaf of the tree this game lands in (§18). The same structure does both, which is why they are not two systems. |
| Embedding space, information clustering — chess and checkers cluster closer than talking to your boss | Similarity is **shared prefix depth weighted by information gain** (§20), with forest proximity (§19.3) as the robust version. Chess/checkers agree on seven axes before diverging; chess and a difficult conversation diverge at axis 0. §20 projects the signatures to a 2D plane by classical MDS, which is the "plane of existence" made literal and is what the frontend renders. |
| Multiple different problems in one | One structure answers all four. The tree's levels are the axes of game-space; descending it is a decision tree; prefix overlap is the clustering metric; a forest of trees with randomised axis orderings is the random forest (§17). |
| An n-dimensional trie — a tree with ending states, with the game encoded into each path | §18. `n` is the number of game-space axes; each contributes one level; a root→node path is the game's **rule signature**; an ending state is a **game class** holding the induced rule set. |
| A radix tree with the mechanics of a trie | §18.1. Path compression from the radix tree so only the axes that actually *discriminate* cost a node; terminal-on-internal nodes from the trie so a general game class can be a strict prefix of a specific one. Neither structure alone can hold both properties. Split and merge follow `RadixCyclicNN/DESIGN.md` §5.2 exactly. |
| Or invert the problem — it could be a random forest | §19. Axis ordering is not unique, so a single tree is a single arbitrary ordering. `T` trees over randomised orderings and bootstrapped corpora, with Breiman proximity as the similarity measure. This is the robust version of §18 and the two agree where both apply. |
| You need an encoder for the game | §21 `GamePackage`: the signature, the learned legal action set, the legality predicate, the GTMNN payoff class and its calibrated constants. That object is the encoding, and it is the entire interface to part two. |
...
2. **The reason text is the highest-bandwidth channel in the system.** Most
   ...
   probes to localise the same rule. §14 treats the oracle's own words as a
...
(§15.4) — which is a tiny fraction of the rule set, because games in the same
...
Induction from nothing still exists in the system (§15.5) as the cold-start path
...
over the axes of §8. Most axes are unobserved most of the time.
...
class Evidence:
    ...
    declared: set[int]              # axes known without probing (§8.1)
...
Nothing in the pipeline requires complete evidence. Retrieval (§18.4) branches
...
declared evidence  ─────────────────────────┐
                                            ▼
                          ┌──────► retrieve candidates (§18.4)
                          │              │
                          │              ▼
                          │      confident enough?  ──yes──►  inherit the winner's rules (§15.4)
                          │              │                              │
                          │              no                             ▼
...
`Explorer` reports the observed rate every epoch (§22.4). A policy running at 5%
...
The implementation computes the full multi-outcome `EIG` (§16.3); the 50% figure
...
`probe` → `axis` → `radix` → `forest` → `similarity` → `package` → `explorer` →
`checkpoint` → `bench` → `cli` → `api`.

---

## 8. The axis catalogue — what we know about a game

An axis is one question about a game with a small discrete answer set. The
catalogue is seeded with 22 and **grows**: `axis.discover()` (§17.3) proposes new
...
### 8.3 What the numbers say, including the part that breaks
...
first. That is a random forest, it is where §19 comes from, and the `chess ↔ go`
...
class Verdict:
    ...
    reason_class: int | None    # filled by reasons.py (§14); None until classified
...
class Action:
    ...
    derivation: tuple | None    # the grammar productions that built it (§10.2), or None
...
downstream must work without it — which is why the probe policies (§16) are
generate-and-score rather than argmax-over-enumeration.

### 10.2 Grammar-driven generation
...
class GrammarActionSpace(ActionSpace):
    ...
    # (§15) searches over productions rather than over characters.
```
...
### 10.3 `neighbours` — the boundary prober

Small edits to a known-legal action land near the boundary, which is where
`p_legal ≈ 0.5` lives (§4). `neighbours` is how the `boundary` policy (§16.5)
...
class Oracle(Protocol):
    ...
    def declared(self) -> dict[int, str]                       # the axes it knows about itself (§8.1)
...
|---|---|---|
| `GrammarOracle` | exact | A known regular or context-free language. **Ground truth is available**, so this is what validates GREN itself (§27). |
...
### 11.2 Why `CompilerOracle` is the reference
...
error index; §14 gets its clusters handed to it rather than having to discover
...
* A timeout or a kill is `Outcome.ERROR`, never `ILLEGAL` (§9).
...
only observed as an unexplained refusal class. `RuleSet.unexplained` (§15) counts
...
and not a failure of the search. §31 keeps this open.

---

## 14. `reasons.py` — the oracle's own words
...
@dataclass
class Rule:
    predicate: Predicate        # from the feature language (§13)
...
class RuleSet:
    rules: list[Rule]
    unexplained: Counter        # reason_class -> probes no rule explains (§13)
...
### 15.1 Induction by information gain
...
same algorithm as the axis ordering (§17.2), pointed at rules instead of games.
...
### 15.4 Inheritance — the point of the whole design
...
def inherit(candidates: "Candidates", corpus: "Corpus", evidence: Evidence) -> RuleSet
    ...
    # Rules held by SOME are inherited at partial confidence and become the probe targets (§16.4):
...
neighbour being right, and `GamePackage.confidence` (§21) is reduced in proportion
...
## 16. `probe.py` — choosing what to try next
...
### 16.4 The candidate-splitting objective
...
The targets are the partially-inherited rules from §15.4 — the ones some
...
### 16.7 `bandit` — the default in `Explorer`
...
AXES: tuple[Axis, ...]           # the §8.1 catalogue, index == id
...
def order(corpus, rng=None, jitter: float = 0.0) -> list[int]
    ...
    # §19 uses to build a forest of genuinely different orderings.
```
...
### 17.2 Ordering by information gain
...
(§15.1) pointed at games instead of refusals, which is why "descending the tree is
...
## 18. `radix.py` — a radix tree with the mechanics of a trie
...
class RadixGameTree:
    ...
    axis_order: list[int]                         # this tree's ordering (§17.2); a forest varies it
...
def merge_child(self, p: int) -> bool
    ...
    # p != c, AND **not terminal[p]**  <-- the trie clause (§18.1). Returns True if merged.
...
### 18.4 `retrieve` — the operation everything else is for
...
class Candidates:
    ...
    general: int | None         # the deepest TERMINAL node every candidate descends from (§18.5)
...
### 18.5 Answering with an internal node
...
it can be packaged and handed to GTMNN (§21) with honest confidence while
...
§8.3 showed a single tree calling chess and go unrelated (prefix 0.08) when they
agree on 71% of the IG-weighted mass, because they differ on one early
high-information axis. One ordering is one arbitrary commitment. The fix is not
to commit.

```python
class GameForest:
    trees: list[RadixGameTree]          # T = 64
    def __init__(self, T: int = 64, jitter: float = 0.5, bootstrap: float = 0.8, seed: int = 0)
        # Tree t is built over axis_order(corpus_sample_t, jitter=jitter) -- a different
        # ordering from a different bootstrap sample of the corpus.
    ...
    def proximity(self, g1: int, g2: int) -> float          # §19.3
    def proximity_matrix(self, games: Sequence[int]) -> list[array]
```

### 19.3 Breiman proximity — the default similarity

```
proximity(g1, g2) = (number of trees where g1 and g2 land in the same terminal node) / T
```

No axis ordering is privileged, so a pair that shares eleven axes and differs on
one is similar in most orderings regardless of where that one axis happens to
sort. `test_forest.py` asserts the regression case directly: **chess ↔ go must
score above 0.6 under forest proximity where prefix similarity gives 0.08.**

Prefix similarity (§20) is retained because retrieval computes it for free and
it is correct when divergence is deep, but it is never the basis of a
`GamePackage`.
...
## 21. `package.py` — the handoff to GTMNN
...
class GamePackage:
    ...
    node: int                           # the tree node it came from; may be INTERNAL (§18.5)
    ...
    payoff_class: str                   # which GTMNN Payoff to instantiate (§21.2)
    B: float ; lam: float               # GTMNN's reward / penalty, calibrated (§21.3)
    confidence: float                   # §21.1
...
### 21.1 Confidence, and the refusal to hand over
...
keeps probing instead. The internal-node package (§18.5) is the graceful path: it
is usually *more* confident than any specific candidate, because it only claims
what they all agree on.

### 21.2 Mapping a signature to a GTMNN payoff

| GREN axes | GTMNN `Payoff` (`GTMNN/DESIGN.md` §8) |
|---|---|
...
| `turn_structure=simultaneous` ∧ `sum=general-sum` | `PrisonersDilemma` / `StagHunt` per §8.3-8.4 |
| no pure equilibrium detected in the corpus neighbour | `RockPaperScissors` structure — GTMNN's cycle machinery and meta-player (§14) are the intended handler |
| `oracle_fidelity=graded` | `BeliefCongestion` only; the correctness game needs a hard label and there is none |

### 21.3 Calibrating `B` and `λ`, and masking repertoires
...
(§14) has collapsed distinct failures into one class.

---

## 23. `checkpoint.py`, `bench.py`
...
* **`GameMap.jsx`** — the centrepiece: the MDS projection (§20) as a scatter plot. Chess and checkers adjacent, rust and python adjacent, the conversation off on its own. Point size by corpus confidence, colour by `category`, hover for the signature, click to pin. This is the "plane of existence" and it is the one screen that shows whether the whole idea works.
* **`TreeView.jsx`** — the radix tree: compressed segments as multi-axis labels on one node, terminal nodes ringed, internal terminals ringed *and* expandable (§18.5), counts as node size, live highlight of the retrieval path during an identification run.
...
* **`AxesPanel.jsx`** — the catalogue by measured IG, with dead axes (IG ≈ 0) flagged for removal and discovered axes (§17.3) marked.
* **`PackagePanel.jsx`** — the `GamePackage`, its confidence decomposed into the three factors of §21.1, and whether GTMNN would accept it.
...
* `test_radix.py` — **a terminal node is never merged away** (§18.1), asserted under randomised insert/compress sequences; split/merge round-trips; `path(insert(σ)) == σ`; an internal node can be terminal and survives compression; `retrieve` on complete evidence returns exactly `lookup`; `retrieve` on empty evidence returns the whole corpus ranked by prior; `to_dict`/`from_dict` round-trips.
* `test_forest.py` — **the chess ↔ go regression (§8.3, §19.3): forest proximity > 0.6 where prefix similarity gives 0.08.** Proximity is symmetric, in `[0,1]`, and 1.0 for a game against itself.
* `test_similarity.py` — chess↔checkers > 0.9 and chess↔boss-conversation < 0.2 on the §8.2 fixtures; MDS preserves the rank order of the top-5 neighbours for 90% of corpus games; MDS is deterministic given a seed.
...
* **W1 — the feature language (§13).** Domain knowledge hides there. A rule outside it cannot be induced. This is the biggest one.
* **W2 — axis commitment.** Axes must be chosen and discretised; §8.3 showed one ordering calling chess and go unrelated. §19 patches it; it does not remove it.
* **W3 — discreteness.** Continuous properties get bucketed, and information is lost at every bucket boundary.

### 29.1 Learned embedding (metric learning) — attacks W2, W3
...
have. And critically, **a vector cannot be inherited from.** §15.4 is the reason
...
rule inheritance and an embedding for similarity, with §20's MDS replaced by a
learned projection once the corpus is large enough to train one.

### 29.2 Bayesian posterior over rule programs — attacks W1, W2, W3
...
approximates the prior's structure, §16.3 is its probe rule exactly. Partial
...
### 29.3 Normalised compression distance — attacks W1 hardest
...
better clusterer. See §31.2.

### 29.4 MinHash over refusal fingerprints — attacks W1, scales
...
### 29.5 A decision tree over probes rather than axes — attacks W2, collapses two modules
...
proxy of axis information gain. §17 and §16 collapse into one module.
*Costs:* the optimal such tree is NP-hard (greedy IG is the standard
approximation, which is what §17.2 already does). Worse, the probes are
...
is meaningful.
*Verdict:* right idea at the wrong level. It is what §16.4 already does *within*
a candidate set, where the action space is shared. As the global structure it
cannot work.

### 29.6 Absorbing Markov chain over the boundary — reuses existing work
...
what routes a game refuses you — which no axis in §8.1 measures. Continuity with
...
### 29.7 Analysis by synthesis — the inversion
...
which is §29.2's problem arriving by a different road.
...
### 29.8 What is actually recommended

Build the tree as specified, and add §29.3's NCD as a cheap independent check
from the first commit. Take §29.6's spectral summaries as additional axes when
the catalogue stops discriminating. Hold §29.1 until the corpus is large enough
to train on, and §29.7 until `unexplained` says it is necessary.
...
## 30. Cross-cutting invariants

1. **A terminal node is never merged away** (§18.1). The one bug this structure is prone to.
2. **`ERROR` never becomes a rule** (§9). Oracle failures do not enter the boundary.
...
5. **Reason classification is order-independent** (§14).
...
8. **Nothing generated is executed outside the sandbox** (§12).

---

## 31. Open questions

1. **Is the feature language (§13) covering the domain?** `RuleSet.unexplained` answers it empirically and nothing else can. Instrumented from the first commit for exactly this reason. If coverage on Rust sits below ~0.6, §29.7 stops being a future direction and becomes the work.
2. **Do the two similarity measures agree?** §29.3's NCD against forest proximity, per game pair, plotted. Systematic disagreement localises where the axis catalogue is blind, and is the only automatic check on W1 available. This is the first experiment to run.
...
5. **Does `B:λ` calibration (§21.3) actually help GTMNN?** It answers GTMNN's own open question 1 from measurement rather than a constant. The test is whether GTMNN's `gini` and `diversity` readouts behave better under a GREN-calibrated ratio than under the default — the first real test of whether the two networks compose or merely concatenate.
6. **Graded oracles.** `oracle_fidelity=graded` is specified and unproven. A refusal that arrives late, partially, and from a party with its own interests is the normal case outside programming, and thresholding it at 0.5 is a placeholder standing where the hard problem is.
